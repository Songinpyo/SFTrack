...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, id_feature):
        ...
        self.init_id_feature = id_feature / np.linalg.norm(id_feature)
    ...
    def activate(self, kalman_filter, frame_id, id_feature):
        ...
        self.id_feature[frame_id] = id_feature / np.linalg.norm(id_feature)
...
class BoTSORT(object):
    def __init__(self, args, frame_rate=30):
        ...
        # Leave removed STracks // 23.04.12 inpyosong
        self.removed_stracks_history = []  # type: list[STrack]
        ...
        self.gmc = GMC(method='sparseOptFlow', verbose=[args.name, False])
        self.reid = False
    ...
    def update(self, output_results, img_info, id_feature, img_size, imgs):
        ...
        if len(dets) > 0:
            '''Detections'''
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, id_f) for
        ...
        # Fix camera motion
        warp = self.gmc.apply(imgs, dets)
        STrack.multi_gmc(strack_pool, warp)
        STrack.multi_gmc(unconfirmed, warp)

        ious_dists = matching.iou_distance(strack_pool, detections)
        ious_dists_mask = (ious_dists > 0.5) # self.proximity_thresh = 0.5
        ...
        if len(dets_second) > 0:
            '''Detections'''
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, id_f) for
        ...
        for i in range(len(removed_stracks)):
            self.removed_stracks_history.append(removed_stracks[i])
